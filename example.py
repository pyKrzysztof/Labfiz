...
def error_k(m, T, NP_M, NP_T): return NP_M/m + 2*NP_T/T
def avg_error_k(error_k): return sum(error_k)/len(error_k)
def max_error_k(error_k): return max(error_k)


workflow = [x, T, k, F, [avg_x, True], [avg_k, True], k_from_avg, error_k, [max_error_k, True], [avg_k_from_avg, True], [avg_error_k, True]]
